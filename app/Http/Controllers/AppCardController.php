...
class AppCardController extends Controller
{
    ...
    public function createRequest()
    {
        $lockers = LocksWReadyCard::where('type','read_card')->where('is_active', true)->get();
        $user = Auth::user();
        return view('app.cards.new_request', compact('lockers', 'user'));
    }
    ...
    public function storeRequest(Request $request)
    {
        ...
        return redirect()->route('min-alqalb.cards.summary');
    }
    ...
    public function showSummary()
    {
        ...
        if (!$pendingLocker || !isset($pendingLocker['locker_id'])) {
            return redirect()->route('min-alqalb.cards.create')
        ...
        return view('app.cards.finish_new_request', compact('locker', 'quantity', 'totalPrice'));
    }
    ...
    public function purchase(Request $request)
    {
        ...
        try {
            ...
            $response = \Illuminate\Support\Facades\Http::withHeaders([
                ...
                'amount' => $amountInFils,
                'currency_code' => 'AED',
                'success_url' => route('min-alqalb.cards.payment.success', ['order_id' => 'lck_' . $payment->id]),
                'cancel_url' => route('min-alqalb.cards.payment.cancel', ['order_id' => 'lck_' . $payment->id]),
    ...
    public function handleSuccess(Request $request)
    {
        ...
        if (!$paymentId) {
            ...
            return redirect()->route('min-alqalb.cards.index')
