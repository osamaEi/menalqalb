...
@section('content')
            ...
            <div class="col-12 col-lg-4 overflow-scroll media-375">
                <div class="All_Button lang contact mt-0 pt-0">
                    ...
                    <form action="{{ route('app.contact.store') }}" method="POST">
                        @csrf
                        <div class="!mt-1">
                            <label for="name" class="localized text-center flex items-center justify-center">اسم المرسل</label>
                        </div>
                        <div class="bg-[#F9F9F9] max-h-[59px] relative rounded-[35px] mt-0 border !border-black">
                            <div class="flex items-center">
                                <input type="text" name="name" id="name" value="{{ old('name', 'Tarek Bn Kalban') }}"
                        ...
                        <div class="!mt-3">
                            <label for="email" class="localized flex items-center justify-center">البريد الإلكتروني</label>
                        </div>
                        <div class="bg-[#F9F9F9] max-h-[59px] relative rounded-[35px] mt-0 border !border-black">
                            <div class="flex items-center">
                                <input type="email" name="email" id="email" value="{{ old('email', 'Tarek@gmail.com') }}"
                                    class="relative right-[-20px] w-[100px] bg-transparent h-[57px] flex-grow text-lg focus:outline-none text-center"
                        ...
                        <div class="!mt-3">
                            <label for="phone" class="localized flex items-center justify-center">رقم الهاتف</label>
                        </div>
                        <div class="bg-[#F9F9F9] max-h-[59px] relative rounded-[35px] mt-0 border !border-black">
                            <div class="flex items-center">
                                <input type="tel" name="phone" id="phone" value="{{ old('phone', '96655555555') }}"
                                    class="relative right-[-20px] w-[100px] bg-transparent h-[57px] flex-grow text-lg focus:outline-none text-center"
                        ...
                        <div class="!mt-3">
                            <label for="account_type" class="localized flex items-center justify-center">نوع الحساب</label>
                        </div>
                      

                        <div class="form-group textarea">
                            <label for="message" class="localized flex items-center justify-center mt-2 !pb-0">رسالة</label>
        ...
        <!-- Confirmation Popup -->
        <div class="popup-overlay" id="confirmationPopup" @if (session('success')) style="display: flex;" @endif>
        ...
        <script src="{{ asset('sass/components/js/jquery-3.3.1.slim.min.js') }}"></script>
        <script src="{{ asset('sass/components/js/popper.min.js') }}"></script>
        <script src="{{ asset('sass/components/js/select2.full.js') }}"></script>
        <script src="{{ asset('sass/components/js/select.js') }}"></script>
        <script src="{{ asset('sass/components/js/bootstrap.min.js') }}"></script>
        <script src="{{ asset('sass/components/js/script.js') }}"></script>
        <script>
